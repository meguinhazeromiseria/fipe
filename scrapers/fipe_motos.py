...
# Config
OUTPUT_FILE = Path("fipe_motos.json")
...
def get_marcas(ref):
    """Lista marcas de motos"""
    data = request_api("ConsultarMarcas", {
        "codigoTabelaReferencia": ref,
        "codigoTipoVeiculo": 2  # Motos
    })
    
    if not data:
        print(f"   ⚠️  API retornou vazio para motos")
        print(f"   Ref: {ref}")
...
def main():
    ...
    print(f"✅ {len(marcas)} marcas de motos")
